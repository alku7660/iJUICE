"""
Feasible Actionable Counterfactual Explanations (FACE)
Based on CARLA Framework implementation: Please see https://github.com/carla-recourse/CARLA/
"""
...
def face_method(counterfactual):
    """
    Function that returns the FACE with respect to instance of interest x
    """

    def graph_search(data, index, keys_immutable, model, n_neighbors=50,
                     p_norm=2, mode="knn", frac=0.4, radius=0.25, undesired_class=0):
        # FACE method from
        ...
        y_positive_indeces = np.where(y_predicted == 1 - undesired_class)
        ...
        for n in neighbors_list:
            neighbor_candidates = find_counterfactuals(candidate_counterfactuals, data, immutable_constraint_matrix1, immutable_constraint_matrix2, index,
                                                       n, y_positive_indeces, is_knn=is_knn)
    ...
    def choose_random_subset(data, frac, index):
        ...
        list_to_choose = (np.arange(0, index).tolist() + np.arange(index + 1, data.values.shape[0]).tolist())
        chosen_indeces = np.random.choice(list_to_choose, replace=False, size=number_samples)
    ...
    def build_constraints(data, i, keys_immutable, epsilon=0.5):
        ...
        """
        immutable_constraint_matrix = np.outer(data[keys_immutable[i]].values + epsilon, data[keys_immutable[i]].values + epsilon)
    ...
    def find_counterfactuals(candidates, data, immutable_constraint_matrix1, immutable_constraint_matrix2, index,
                             n, y_positive_indeces, is_knn):
        """
        ...
        graph = build_graph(data, immutable_constraint_matrix1, immutable_constraint_matrix2, is_knn, n)
    ...
    data_df_copy = pd.concat([factuals, data_df_copy], ignore_index=True)
    data_df_copy = data_df_copy[order_features]
    factuals = factuals[order_features]

    cf = graph_search(data_df_copy, 0, counterfactual.data.immutables, counterfactual.model.model, mode=mode, 
